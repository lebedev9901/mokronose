...
const petIdInput = document.getElementById('catalogPetId');
...
function clearPetFilterChecks() {
    document.querySelectorAll('input[name="age_group[]"]').forEach(input => {
        input.checked = false;
    });

    document.querySelectorAll('input[name="breed_size[]"]').forEach(input => {
        input.checked = false;
    });
}

function applyPetFilterChecks(option) {
    clearPetFilterChecks();

    if (!option || !option.value) {
        return;
    }

    const age = option.dataset.age || 'all';
    const size = option.dataset.size || 'all';
...
if (petPickBtn && petSelect) {
    ...
    petSelect.addEventListener('change', function () {
        ...
        if (petIdInput) {
            petIdInput.value = this.value;
        }

        applyPetFilterChecks(option);
        loadCatalog();
    });

    if (petSelect.value) {
        applyPetFilterChecks(petSelect.options[petSelect.selectedIndex]);
    }
}
